...
class IndpTest_naive_rff(IndpTest):
    ...
    def perform_test(self, rff_num):
        ...
        rff_num (D): the number of frequency samplings 频率采样的数目--傅里叶特征数---随机傅里叶特征的维度（默认500）
        """
        if self.kernel_type == "Gaussian":  #仅支持高斯核#
            """ generate frequency """
            dx = self.X.shape[1]
            dy = self.Y.shape[1]
            ...
            """选择配置计算阈值 T-gamma分布近似  F-使用排列检验计算阈值（##）"""
            if self.null_gamma == True:
                fX, fY = self.feat_gen_gaussian(self.X, self.Y, wx, wy)   #计算高斯特征核#
                rfx, rfy = self.rff_generate(fX, fY, unit_rff_freqx_fix, unit_rff_freqy_fix)     #生成随机傅里叶特征#
                #中心化特征#
    ...
    def cal_thresh_pm(self, X, Y, wx, wy):
        ind = []
        for _ in range(self.n_permutation): #1-n_permutation=100#
            p = np.random.permutation(len(X)) #随机对序列{0-len(x)}进行排序---随机序列索引#
            Xp = X[p] #  ·ndArray同类型多维数组#   ·通过 X[p]，X 中的元素会按照 p 的顺序重新排列---对行（第一维）进行重排（行互换）#
    ...
    def rff_generate(self, fX, fY, unit_rff_freqx, unit_rff_freqy):
        ...
        #对dotw逐个元素求cos和sin值 拼接起来-按列torch.cat(,,1)第一维-按列 #
    ...
    def freq_gen_gaussian(self, dx, dy, rff_num = 500):
        #/size参数其实是（int(rff_num / 2), dx）传参规范没把他们扩在一起 就是这个意思其实/#
